...
export default function SeriesPage() {
  ...
  <>
      ...
      <WRAPPER className="mb-8">
        <WRAPPER className="bg-card rounded-xl shadow-sm border border-border p-4">
      ...
      {filteredSeries.length > 0 ? (
        <WRAPPER className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSeries.map((seriesItem) => (
            ...
            >
              <WRAPPER className="aspect-video bg-muted relative">
                {seriesItem.imageUrl ? (
              ...
              <WRAPPER className="p-6">
                <WRAPPER className="flex items-start justify-between mb-3">
                ...
                <WRAPPER className="flex items-center text-sm text-muted-foreground mb-4">
        ...
        <WRAPPER className="text-center py-12 bg-card rounded-xl border border-border">
...
`.replaceAll('WRAPPER', 'div');

fs.writeFileSync(path.join(process.cwd(), 'app/series/page.tsx'), content);
console.log('series page done');
